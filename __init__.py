# CrateTools PreviewVideoMonitorPro v5.3 - ComfyUI skin (visual only) over v5.2 engine - August 2026
# GPU-accelerated
...
# =====================================================================
# COMFYUI SKIN (v5.3) - visual tokens only, zero behaviour change
#
# Palette mirrors ComfyUI_frontend (dark theme):
#   packages/design-system/src/css/_palette.css
#   packages/design-system/src/css/style.css  (.dark-theme block)
#   src/constants/slotColors + docs/interface/appearance (node_slot colors)
# Every RGB literal in the UI now references one of these names so the
# skin can be tuned in one place. Scope CONTENT (histogram/vectorscope/
# waveform traces, graticules, zebra, broadcast guides, picker readouts)
# is intentionally NOT tokenised - those are measurement colours.
# =====================================================================
class UI:
    # --- neutral ramp (blue-tinted charcoal) ---
    CHARCOAL_800 = (23, 23, 24)      # #171718 panel / toolbar / menu surface
    CHARCOAL_700 = (32, 33, 33)      # #202121 header surface
    CHARCOAL_600 = (38, 39, 41)      # #262729 button surface / body
    CHARCOAL_500 = (45, 46, 50)      # #2d2e32 divider
    CHARCOAL_400 = (49, 50, 53)      # #313235 interface stroke / hover surface
    CHARCOAL_300 = (60, 61, 66)      # #3c3d42 selected surface
    CHARCOAL_200 = (73, 74, 80)      # #494a50 border default
    CHARCOAL_100 = (85, 86, 94)      # #55565e muted background
    ASH_800 = (68, 68, 68)           # #444444 node stroke / inactive outline
    ASH_500 = (130, 130, 130)        # #828282
    SMOKE_800 = (138, 138, 138)      # #8a8a8a muted foreground
    SMOKE_700 = (160, 160, 160)      # #a0a0a0 secondary text
    SMOKE_500 = (197, 197, 197)      # #c5c5c5
    SMOKE_300 = (225, 225, 225)      # #e1e1e1
    SMOKE_200 = (233, 233, 233)      # #e9e9e9 primary UI text
    WHITE = (255, 255, 255)
    BLACK = (0, 0, 0)
    SHADOW = (12, 12, 13)            # opaque stand-in for rgba(0,0,0,.4) drop shadow

    # --- accents ---
    AZURE_600 = (11, 140, 233)       # #0b8ce9 primary / selected / executing
    AZURE_400 = (49, 185, 244)       # #31b9f4 primary hover / "cyan" feedback
    AZURE_300 = (120, 186, 233)      # #78bae9
    COBALT_800 = (24, 90, 139)       # #185a8b primary fill (dim)
    JADE_600 = (0, 205, 114)         # #00cd72 success
    JADE_400 = (71, 228, 105)        # #47e469
    JADE_DIM = (0, 82, 46)           # jade-600 @ 40% over charcoal (success fill)
    JADE_DIM_HOVER = (0, 110, 61)
    GOLD_600 = (253, 153, 3)         # #fd9903 warning / orange
    GOLD_500 = (253, 171, 52)        # #fdab34 attention yellow
    GOLD_400 = (252, 191, 100)       # #fcbf64
    GOLD_DEEP = (196, 118, 0)        # darker gold for text on dark
    CLIP_YELLOW = (255, 213, 0)      # #ffd500 node_slot CLIP / video-trim selection
    CLIP_YELLOW_DIM = (179, 149, 0)  # #ffd500 @ 70% (trim range fill)
    CORAL_700 = (179, 58, 58)        # #b33a3a destructive
    CORAL_600 = (224, 78, 72)        # #e04e48 destructive hover / outline
    CORAL_500 = (247, 89, 81)        # #f75951
    ERROR = (150, 42, 42)            # #962a2a error stroke (destructive fill)
    ERROR_DIM = (84, 32, 33)         # error @ 55% over charcoal
    PLAYHEAD_RED = (240, 81, 59)     # #f0513b video-trim playhead
    PLUM_400 = (141, 127, 197)       # #8d7fc5 fit-mode outline (violet)
    PLUM_300 = (175, 163, 219)       # #afa3db
    OCEAN_600 = (47, 104, 122)       # #2f687a teal selection

    # --- ComfyUI node slot colours (used as channel tints) ---
    SLOT_VAE_RED = (255, 110, 110)   # #FF6E6E  -> "R"
    SLOT_MASK_GREEN = (129, 199, 132)  # #81C784 -> "G"
    SLOT_IMAGE_BLUE = (100, 181, 246)  # #64B5F6 -> "B"

    # --- alpha variants (RGBA) ---
    PANEL_ALPHA = (23, 23, 24, 230)      # vision / scope / info panels
    SCOPE_PANEL_ALPHA = (23, 23, 24, 204)
    TOOLBAR_ALPHA = (23, 23, 24, 204)  # glass dock: 80% opacity
    DIVIDER_ALPHA = (49, 50, 53, 255)
    DOCK_STROKE_ALPHA = (73, 74, 80, 255)   # charcoal-200: ComfyUI's panel border
    BTN_ALPHA = (38, 39, 41, 255)
    BTN_ACTIVE_ALPHA = (24, 90, 139, 255)
    BTN_BORDER_ALPHA = (73, 74, 80, 255)
    BTN_BORDER_ACTIVE_ALPHA = (11, 140, 233, 255)
    TEXT_ALPHA = (233, 233, 233, 255)
    TEXT2_ALPHA = (160, 160, 160, 255)
    TEXT3_ALPHA = (138, 138, 138, 255)
    SLIDER_BAR_ALPHA = (49, 50, 53, 255)
    SLIDER_TICK_ALPHA = (85, 86, 94, 255)
    SLIDER_HANDLE_ALPHA = (247, 89, 81, 255)
    CORAL_TEXT_ALPHA = (247, 89, 81, 255)

    # --- shape ---
    RADIUS_BTN = 6       # Comfy rounded-md
    RADIUS_MENU = 8
    RADIUS_DOCK = 12     # Comfy rounded-xl (actionbar / nodes)
    RADIUS_PILL = 8      # sub-pill groups inside the dock
    STROKE_BTN = 2       # Comfy uses thin strokes (3px only for selection)
    DOCK_INSET = 10      # floating dock: gap to window edges
    PILL_PAD = 6         # padding inside a sub-pill
    PILL_GAP = 8         # gap between sub-pills
    PILL_H = 32          # sub-pill height (buttons are 30)


# ---------------------------------------------------------------------
# Font loader: Inter (ComfyUI's UI typeface). Drop Inter-Regular.ttf and
# Inter-Medium.ttf into <node>/fonts/ (SIL OFL, https://rsms.me/inter/).
# Falls back to system Inter -> Segoe UI / Arial -> pygame default so the
# monitor never fails to start because of a missing font file.
# Sizes are scaled slightly because Inter has a larger x-height than
# pygame's default freesansbold; keeps button widths close to v5.2.
# ---------------------------------------------------------------------
FONTS_DIR = os.path.join(BASE_DIR, "fonts")
INTER_SIZE_SCALE = 0.9
_FONT_CACHE = {}
_FONT_FILES = {
...
def ui_font(size, weight="regular"):
    """Return a cached pygame Font in the ComfyUI typeface (Inter)."""
    key = (size, weight)
    ...
    if PYGAME_AVAILABLE:
        ...
        try:
            if os.path.exists(path):
                f = pygame.font.Font(path, max(6, int(round(size * INTER_SIZE_SCALE))))
        except Exception:
            f = None
        if f is None:
            try:
                f = pygame.font.SysFont("Inter, Segoe UI, Arial", size, bold=False)
            except Exception:
...
class PreviewVideoMonitorPro:
    ...
    def __init__(self):
        ...
        self.toolbar_height = 80   # 70px dock + 10px floating gap (UI.DOCK_INSET)
        self.timeline_height = 19  # v5.3: shorter bar, top edge unchanged
    ...
    def _draw_gamuts_button(self, screen, position, is_active=False, is_hover=False):
        ...
        if is_active or self.color_space_dropup_open:
            bg_color = UI.CHARCOAL_400  # Medium gray for active/open
        elif is_hover:
            bg_color = UI.CHARCOAL_400  # Medium gray for hover
        else:
            bg_color = UI.CHARCOAL_600  # Default dark gray

        # Border color - GREEN outline when selected (like Pong button)
        if self.color_space_dropup_open:
            border_color = UI.AZURE_600  # open
        else:
            border_color = UI.ASH_800  # Gray when not open
        ...
        # Red line (left)
        red_x = start_x
        pygame.draw.line(screen, UI.SLOT_VAE_RED, 
                        (red_x, position[1] + 6), 
        ...
        pygame.draw.line(screen, UI.SLOT_MASK_GREEN, 
        ...
        pygame.draw.line(screen, UI.SLOT_IMAGE_BLUE, 
                        (blue_x, position[1] + 6), 
    ...
    def _draw_color_space_dropup(self, screen, mouse_pos, gamuts_button_rect=None):
        ...
        if font_key not in self._ui_cache:
            self._ui_cache[font_key] = ui_font(18)
        font = self._ui_cache[font_key]

        # Background
        menu_rect = pygame.Rect(menu_x, menu_y, menu_width, menu_height)
        pygame.draw.rect(screen, UI.CHARCOAL_800, menu_rect, border_radius=UI.RADIUS_MENU)
        pygame.draw.rect(screen, UI.CHARCOAL_200, menu_rect,
                         width=1, border_radius=UI.RADIUS_MENU)
        ...
        for i in range(total_items):
            ...
            if is_selected:
                ...
                pygame.draw.rect(screen, UI.COBALT_800,
                                 item_rect, border_radius=3)
                pygame.draw.rect(screen, UI.PLUM_400,
                                 item_rect, width=2, border_radius=3)
            elif is_hover:
                # Gray background for hover
                pygame.draw.rect(screen, UI.CHARCOAL_400,
                                 item_rect, border_radius=3)
            ...
            if text_key not in self._ui_cache:
                if is_selected:
                    text_color = UI.PLUM_300  # Light violet for selected
                else:
                    text_color = UI.SMOKE_200  # Normal white
    ...
    def _draw_vision_module(self, screen, mouse_pos):
        ...
        # Surface is taller than the module so the cap can overlap the dock's
        # square top edge (hides the seam between the two tiers)
        module_surface = pygame.Surface((screen_width, module_height + UI.RADIUS_DOCK), pygame.SRCALPHA)
        
        # v5.3 dock: Vision tier is the rounded cap of the floating dock.
        # Geometry of sliders / toggles below is unchanged (hit-tests mirror it).
        inset = UI.DOCK_INSET
        cap = pygame.Rect(inset, 0, screen_width - 2 * inset, module_height + UI.RADIUS_DOCK)
        # shadow
        for i, a in ((3, 18), (2, 30), (1, 45)):
            sh = cap.inflate(i * 2, 0).move(0, -i)
            pygame.draw.rect(module_surface, (0, 0, 0, a), sh,
                             border_top_left_radius=UI.RADIUS_DOCK + i,
                             border_top_right_radius=UI.RADIUS_DOCK + i)
        pygame.draw.rect(module_surface, UI.TOOLBAR_ALPHA, cap,
                         border_top_left_radius=UI.RADIUS_DOCK, border_top_right_radius=UI.RADIUS_DOCK)
        pygame.draw.rect(module_surface, UI.DOCK_STROKE_ALPHA, cap, width=1,
                         border_top_left_radius=UI.RADIUS_DOCK, border_top_right_radius=UI.RADIUS_DOCK)
        ...
        # Two sub-pills: sliders | scopes-channels-guides
        pill_top, pill_h = 5, module_height - 10
        # One sub-pill per slider (bars run from slider_x+10 to slider_x+width-10)
        _sw = slider_section_width // 3
        for i in range(3):
            sx = 10 + _sw * i
            left = max(sx + 2, inset + UI.PILL_PAD)
            right = min(sx + _sw - 2, slider_section_width + 2)
            self._draw_rounded_outline(module_surface, pygame.Rect(left, pill_top, right - left, pill_h),
                                       UI.CHARCOAL_700 + (255,), None, radius=UI.RADIUS_PILL)
        ...
        font_key = "font_16"
        if font_key not in self._ui_cache:
            self._ui_cache[font_key] = ui_font(16)
        font = self._ui_cache[font_key]
        ...
        for i, scope_label in enumerate(scope_buttons):
            ...
            # Button colors
            if is_active:
                bg_color = UI.BTN_ACTIVE_ALPHA  # Blue when active
                border_color = UI.BTN_BORDER_ACTIVE_ALPHA  # Bright blue border
            else:
                bg_color = UI.BTN_ALPHA  # Dark grey when inactive
                border_color = UI.BTN_BORDER_ALPHA  # Grey border
            
            # Draw button
            self._draw_rounded_outline(module_surface, pygame.Rect(button_rect), bg_color, border_color, radius=5, width=2)
            
            # Draw label
            label_text = font.render(scope_label, True, UI.TEXT_ALPHA)
        ...
        # Draw Zebra button
        self._draw_rounded_outline(module_surface, pygame.Rect(zebra_button_rect), bg_color, border_color, radius=5, width=2)
        
        # Draw Z label
        zebra_label_text = font.render("Z", True, UI.TEXT_ALPHA)
        ...
        # Draw Picker button
        self._draw_rounded_outline(module_surface, pygame.Rect(picker_button_rect), bg_color, border_color, radius=5, width=2)
        
        # Draw P label
        picker_label_text = font.render("P", True, UI.TEXT_ALPHA)
        ...
        channel_buttons = [
            ("R", "Red", UI.SLOT_VAE_RED + (255,), 5),      # Red button
            ("G", "Green", UI.SLOT_MASK_GREEN + (255,), 6),    # Green button
            ("B", "Blue", UI.SLOT_IMAGE_BLUE + (255,), 7)      # Blue button
        ]
        ...
        for label, mode, color, offset in channel_buttons:
            ...
            # Button colors
            if is_active:
                bg_color = (60, 61, 66, 255)  # Darker grey when active
        ...
        # Draw Guides button
        self._draw_rounded_outline(module_surface, pygame.Rect(guides_button_rect), bg_color, border_color, radius=5, width=2)
        
        # Draw custom 4-line grid icon (not touching edges)
        icon_color = UI.TEXT_ALPHA
        icon_padding = 8
        # Vertical lines
        ...
        # Draw Mask button
        self._draw_rounded_outline(module_surface, pygame.Rect(mask_button_rect), bg_color, border_color, radius=5, width=2)
        
        # Draw M label
        mask_label_text = font.render("M", True, UI.TEXT_ALPHA)
        ...
        # Draw Flip button
        self._draw_rounded_outline(module_surface, pygame.Rect(flip_button_rect), bg_color, border_color, radius=5, width=2)
        ...
        # Blit the Vision tier flush with the dock top (no overlap: with the
        # glass alpha, overlapping rows double-composite into a dark line;
        # the dock erases its own top stroke while Vision is open)
        screen.blit(module_surface, (0, module_y),
                    area=pygame.Rect(0, 0, screen_width, module_height))
    ...
    def _draw_vision_slider(self, screen, font, label, value, x, y, width, height, mouse_pos, range_type="exposure"):
        ...
        # v5.3 skin: ~10% smaller (14 vs 16) and hugging the top edge so the
        # text sits clear of the needle / tick row below it.
        label_font = ui_font(12)
        label_text = label_font.render(label, True, UI.TEXT2_ALPHA)
        value_text = label_font.render(f"   {display_value:.1f}", True, UI.TEXT_ALPHA)  # 3 spaces before value
        ...
        label_y = y + 4  # inside the slider's sub-pill (top at 5), clear of the needle
        
        # Draw label
        screen.blit(label_text, (start_x, label_y))
        
        # Draw value right after label
        screen.blit(value_text, (start_x + label_text.get_width(), label_y))
        
        # Slider bar dimensions
        bar_y = y + label_height + 20  # v5.3: track sits 5px lower in its sub-pill
        bar_height = 2  # thinner track (v5.3)
        # +5px inset per side so the track floats inside its sub-pill
        bar_x_start = x + padding + 5
        bar_x_end = x + width - padding - 5
        bar_width = bar_x_end - bar_x_start
        
        # Draw slider bar background
        bar_rect = pygame.Rect(bar_x_start, bar_y, bar_width, bar_height)
        pygame.draw.rect(screen, UI.SLIDER_BAR_ALPHA, bar_rect, border_radius=2)
        ...
        for i in range(num_ticks + 1):  # +1 to include both ends
            ...
            pygame.draw.polygon(screen, UI.SLIDER_TICK_ALPHA, tick_points)  # Grey ticks
        ...
        pygame.draw.polygon(screen, UI.BTN_BORDER_ACTIVE_ALPHA, manipulator_points)  # Red - on top of ticks!
    ...
    def _draw_histogram_scope(self, screen, frame_surface):
        ...
        # Scope dimensions (1/4 width, floating panel inset from the edges)
        inset = UI.DOCK_INSET
        scope_width = int(screen_width * 0.25)
        scope_height = screen_height - self.toolbar_height - 50 - 2 * inset  # above Vision tier
        scope_x = screen_width - scope_width - inset
        scope_y = inset
        ...
        bg_color = UI.SCOPE_PANEL_ALPHA  # Dark background, 80% opaque
        pygame.draw.rect(scope_surface, bg_color, (0, 0, scope_width, scope_height), border_radius=UI.RADIUS_DOCK)
        
        # Border
        pygame.draw.rect(scope_surface, UI.BTN_BORDER_ACTIVE_ALPHA, (0, 0, scope_width, scope_height), width=2, border_radius=UI.RADIUS_DOCK)
        ...
        try:
            ...
            font_key = "font_14"
            if font_key not in self._ui_cache:
                self._ui_cache[font_key] = ui_font(14)
            small_font = self._ui_cache[font_key]
            
            label_text = small_font.render("HISTOGRAM", True, UI.TEXT2_ALPHA)
            scope_surface.blit(label_text, (10, 10))
            
            # Mark 0 and 255 (black and white points): small, sitting in the
            # bottom margin below the graph, clear of the corner radius
            scale_font = ui_font(11)
            zero_text = scale_font.render("0", True, UI.TEXT3_ALPHA)
            max_text = scale_font.render("255", True, UI.TEXT3_ALPHA)
            label_y = scope_height - zero_text.get_height() - 5
            scope_surface.blit(zero_text, (16, label_y))
            scope_surface.blit(max_text, (scope_width - max_text.get_width() - 16, label_y))
            ...
            if font_key not in self._ui_cache:
                self._ui_cache[font_key] = ui_font(14)
            small_font = self._ui_cache[font_key]
            error_text = small_font.render("Histogram Error", True, UI.CORAL_TEXT_ALPHA)
    ...
    def _draw_vectorscope(self, screen, frame_surface):
        ...
        # Scope dimensions - SQUARE (size = 1/4 screen width), floating panel
        inset = UI.DOCK_INSET
        scope_size = int(screen_width * 0.25)
        scope_x = screen_width - scope_size - inset
        scope_y = inset
        
        # Create semi-transparent square surface (80% opacity)
        scope_surface = pygame.Surface((scope_size, scope_size), pygame.SRCALPHA)
        bg_color = UI.SCOPE_PANEL_ALPHA  # Dark background, 80% opaque
        pygame.draw.rect(scope_surface, bg_color, (0, 0, scope_size, scope_size), border_radius=UI.RADIUS_DOCK)
        
        # Border
        pygame.draw.rect(scope_surface, UI.BTN_BORDER_ACTIVE_ALPHA, (0, 0, scope_size, scope_size), width=2, border_radius=UI.RADIUS_DOCK)
        ...
        label_text = small_font.render("VECTORSCOPE", True, UI.TEXT2_ALPHA)
        scope_surface.blit(label_text, (10, 10))
    ...
    def _draw_waveform_rgb(self, screen, frame_surface):
        ...
        label_text = small_font.render("WAVEFORM RGB", True, UI.TEXT2_ALPHA)
    ...
    def _draw_aspect_mask(self, screen, frame_pos):
        ...
        try:
            ...
            ratio_text = small_font.render(f"Mask: {mask_name}", True, (197, 197, 197, 255))
            text_x = pos_x + 10
    ...
    def _draw_guides_overlay(self, screen, frame_pos):
        ...
        try:
            ...
            thirds_color = UI.AZURE_600 + (180,)  # ComfyUI azure
            ...
            action_color = UI.AZURE_600 + (180,)  # ComfyUI azure
            ...
            title_color = UI.AZURE_600 + (180,)  # ComfyUI azure
            ...
            cross_size = 10
            center_color = UI.AZURE_600 + (255,)  # ComfyUI azure
    ...
    def _draw_color_picker(self, screen, frame_pos):
        ...
        try:
            ...
            bg_color = UI.SCOPE_PANEL_ALPHA  # 80% opaque like Vision bar
            info_surface.fill(bg_color)
            
            # Border
            pygame.draw.rect(info_surface, UI.BTN_BORDER_ALPHA, (0, 0, info_panel_width, info_panel_height), width=1)
            ...
            sat_text = picker_font.render(f"SAT:{sat:.2f}", True, UI.TEXT2_ALPHA)
            luma_text = picker_font.render(f"LUMA:{luma}", True, UI.TEXT2_ALPHA)
    ...
    def _draw_snapshot_dropup(self, screen, mouse_pos, snapshot_button_rect=None):
        ...
        take_bg = UI.JADE_DIM_HOVER if take_hover else UI.JADE_DIM  # Dark green background
        
        pygame.draw.rect(screen, take_bg, take_rect, border_radius=3)
        pygame.draw.rect(screen, UI.JADE_600, take_rect, width=2, border_radius=3)  # Light grey outline (always)

        # Draw Clear button with RED background and outline (like ClearCache)
        clear_bg = UI.ERROR if clear_hover else UI.ERROR_DIM
        pygame.draw.rect(screen, clear_bg, clear_rect, border_radius=3)
        pygame.draw.rect(screen, UI.CORAL_600, clear_rect, width=2, border_radius=3)

        # Draw Explore button
        explore_bg = UI.CHARCOAL_600
        if explore_hover:
            explore_bg = UI.CHARCOAL_400
        
        pygame.draw.rect(screen, explore_bg, explore_rect, border_radius=3)
        pygame.draw.rect(screen, UI.CHARCOAL_200, explore_rect, width=2, border_radius=3)

        # Draw text - change to confirmation if needed
        take_text = font.render("Take", True, UI.SMOKE_200)
        if self.snapshot_clear_confirm:
            clear_text = font.render("¿ClearForever?", True, UI.WHITE)  # Confirmation text
        else:
            clear_text = font.render("Clear Snaps", True, UI.WHITE)  # Normal text
        explore_text = font.render("Explore", True, UI.SMOKE_200)
    ...
    def _draw_fps_dropup(self, screen, mouse_pos, fps_button_rect=None):
        ...
        menu_rect = pygame.Rect(menu_x, menu_y, menu_width, menu_height)
        pygame.draw.rect(screen, UI.CHARCOAL_800, menu_rect, border_radius=UI.RADIUS_MENU)
        pygame.draw.rect(screen, UI.CHARCOAL_200, menu_rect, width=1, border_radius=UI.RADIUS_MENU)  # Grey border
        ...
        for i, fps_value in enumerate(self.fps_presets):
            ...
            # Background color
            if is_current:
                bg_color = UI.AZURE_600  # ComfyUI selection fill for current fps
            elif is_hover:
                bg_color = UI.CHARCOAL_400  # Grey hover
            else:
                bg_color = UI.CHARCOAL_600  # Default dark
            ...
            text_surf = font.render(fps_text, True, UI.SMOKE_200)
        ...
        pygame.draw.line(screen, UI.CHARCOAL_200, 
        ...
        # Background
        fps_field_bg = UI.CHARCOAL_300 if self.fps_custom_editing else UI.CHARCOAL_600
        pygame.draw.rect(screen, fps_field_bg, fps_rect, border_radius=2)
        # YELLOW outline for FPS field
        pygame.draw.rect(screen, UI.GOLD_500, fps_rect, width=2, border_radius=2)
        ...
        if self.fps_custom_editing:
            ...
            pygame.draw.line(screen, UI.SMOKE_200, 
    ...
    def _draw_clearcache_dropup(self, screen, mouse_pos, clearcache_button_rect=None):
        ...
        if self.clearcache_clear_confirm:
            ...
            clear_text = font.render("Clear Gens", True, UI.WHITE)  # Normal text
        explore_text = font.render("Explore", True, UI.SMOKE_200)
    ...
    def _exit_fullscreen(self, screen):
        ...
        if self.fullscreen_mode:
            ...
            pygame.display.set_caption(
                "Preview Video Monitor Pro v5.3 - [q] Close | [SPACE] Play/Pause | [←→] Step | [ENTER] Edit/Confirm | [↑↓] Navigate | [END] Load | [1-5] Fit Modes | [i/o/p] In/Out Marks | Click timeline to scrub | Right-click generation to rename")
            return screen
    ...
    def _get_or_create_frame_surface(self, screen, current_frame, total_frames):
        ...
        try:
            vw, vh = screen.get_size()
            th = 0  # glass dock: video letterboxes to the full window; dock floats over
    ...
    def _draw_in_button(self, screen, position, is_active=False, is_hover=False):
        ...
        if is_active or self.user_marks_active:
            ...
            bg_color = UI.CHARCOAL_300
            border_color = UI.AZURE_600  # ComfyUI selection ring
            text_color = UI.GOLD_500
        elif is_hover:
            bg_color = UI.CHARCOAL_400  # Medium gray for hover
            border_color = UI.GOLD_500  # Yellow border
            text_color = UI.GOLD_DEEP  # Darker yellow text
        else:
            bg_color = UI.CHARCOAL_600  # Default dark gray
            border_color = UI.GOLD_500  # Yellow border
            text_color = UI.GOLD_DEEP  # Darker yellow text

        # Rounded-square button (matches the view group)
        shadow_rect = btn_rect.move(1, 1)
        pygame.draw.rect(screen, UI.SHADOW, shadow_rect, border_radius=UI.RADIUS_BTN)
        self._draw_rounded_outline(screen, btn_rect, bg_color, border_color)

        # Draw 'i' text
        font_key = f"font_20"
        if font_key not in self._ui_cache:
            self._ui_cache[font_key] = ui_font(20)
        font = self._ui_cache[font_key]
        ...
        # centre the visible ink of the glyph (lowercase 'o' has empty ascender space)
        ink = text_surf.get_bounding_rect()
        cx, cy = position[0] + btn_radius, position[1] + btn_radius
        screen.blit(text_surf, (cx - ink.x - ink.width // 2, cy - ink.y - ink.height // 2))
    ...
    def _draw_out_button(self, screen, position, is_active=False, is_hover=False):
        ...
        if is_active or self.user_marks_active:
            ...
            bg_color = UI.CHARCOAL_300
            border_color = UI.AZURE_600  # ComfyUI selection ring
            text_color = UI.CORAL_600
        elif is_hover:
            bg_color = UI.CHARCOAL_400  # Medium gray for hover
            border_color = UI.CORAL_600  # Red border
            text_color = UI.CORAL_700  # Darker red text
        else:
            bg_color = UI.CHARCOAL_600  # Default dark gray
            border_color = UI.CORAL_600  # Red border
            text_color = UI.CORAL_700  # Darker red text
    ...
    def _draw_fps_button(self, screen, position, is_active=False, is_hover=False):
        ...
        if is_active or self.fps_dropup_open:
            bg_color = UI.CHARCOAL_300  # Slightly blue-grey when dropup open
        elif is_hover:
            bg_color = UI.CHARCOAL_400  # Medium gray for hover
        else:
            bg_color = UI.CHARCOAL_600  # Default dark gray

        # Border color - azure when open, grey otherwise
        border_color = UI.AZURE_600 if (is_active or self.fps_dropup_open) else UI.ASH_800

        # Rounded-square button (matches the I/O and view-group buttons)
        shadow_rect = btn_rect.move(1, 1)
        pygame.draw.rect(screen, UI.SHADOW, shadow_rect, border_radius=UI.RADIUS_BTN)
        self._draw_rounded_outline(screen, btn_rect, bg_color, border_color)
        ...
        dot_color = UI.SMOKE_700  # Light grey dot
        pygame.draw.circle(
    ...
    def _draw_arrow_button(self, screen, direction, position, is_active=False, is_hover=False):
        ...
        # Background color based on state
        if is_active:
            bg_color = UI.CHARCOAL_300  # Dark blue-gray for active state
        elif is_hover:
            bg_color = UI.CHARCOAL_400  # Medium gray for hover
        else:
            bg_color = UI.CHARCOAL_600  # Default dark gray
        ...
        if is_active:
            border_color = UI.AZURE_600  # ComfyUI selection colour
        else:
            border_color = UI.ASH_800  # Light grey outline when inactive

        # Draw button with enhanced outline
        # Shadow first (under the button) so the outline stays continuous
        shadow_rect = btn_rect.move(1, 1)
        pygame.draw.rect(screen, UI.SHADOW, shadow_rect, border_radius=UI.RADIUS_BTN)
        self._draw_rounded_outline(screen, btn_rect, bg_color, border_color)

        # Draw arrow triangle - cached
        triangle_color = UI.SMOKE_200
    ...
    def _draw_button(self, screen, text, position, is_active=False, is_hover=False, color_override=None):
        ...
        if state_key not in self._ui_cache:
            text_surf = font.render(text, True, UI.SMOKE_200)
        ...
        # Centre vertically on CAP-HEIGHT metrics shared by every label, so
        # baselines line up across buttons (per-label ink-centring made
        # descender words like "Snapshot" sit visibly higher than "Vision")
        cap_key = "cap_metrics_20"
        cap = self._ui_cache.get(cap_key)
        if cap is None:
            cap = font.render("H", True, UI.SMOKE_200).get_bounding_rect()
            self._ui_cache[cap_key] = cap
        text_rect = text_surf.get_rect()
        text_rect.x = button_rect.centerx - text_rect.width // 2
        text_rect.y = button_rect.centery - cap.y - cap.height // 2
        ...
        if is_active:
            if color_override:
                ...
                # ComfyUI skin: one selection colour for every active button
                border_color = UI.AZURE_600
        else:
            # Light grey outline when inactive
            border_color = UI.ASH_800

        # Draw button with enhanced outline (4 pixels thick)
        # Shadow first (under the button) so the outline stays continuous
        shadow_rect = button_rect.move(1, 1)
        pygame.draw.rect(screen, UI.SHADOW, shadow_rect, border_radius=UI.RADIUS_BTN)
        self._draw_rounded_outline(screen, button_rect, bg_color, border_color)
    ...
    def _draw_wipe_overlay(self, screen, current_frame_surface, current_frame_pos):
        ...
        if not self.wipe_dragging:
            ...
            pygame.draw.line(screen, UI.GOLD_500,
                           (wipe_x_screen, vid_y), 
            ...
            square_size = 8
            pygame.draw.rect(screen, UI.GOLD_400,
    ...
    def _draw_sbs_overlay(self, screen, current_frame_surface, current_frame_pos):
        ...
        pygame.draw.line(screen, UI.AZURE_600,
                       (half_width, 0),
                       (half_width, vh),
                       width=2)
    ...
    def _draw_dropup_menu(self, screen, mouse_pos, generations_button_rect=None):
        ...
        for i in range(visible_items):
            ...
            pygame.draw.rect(screen, UI.CHARCOAL_600, wipe_btn_rect, border_radius=2)
            
            # Draw wipe button square with different colors for each mode
            if is_sbs_selected:
                # BLUE fill for side-by-side mode (Shift+Click)
                pygame.draw.rect(screen, UI.AZURE_600, wipe_btn_rect, border_radius=2)
                pygame.draw.rect(screen, UI.AZURE_400, wipe_btn_rect, width=1, border_radius=2)
            elif is_wipe_selected:
                # YELLOW fill for wipe mode (normal click)
                pygame.draw.rect(screen, UI.GOLD_500, wipe_btn_rect, border_radius=2)
                pygame.draw.rect(screen, UI.GOLD_400, wipe_btn_rect, width=1, border_radius=2)
            else:
                # Light grey square for unselected
                pygame.draw.rect(screen, UI.CHARCOAL_100, wipe_btn_rect, border_radius=2)
                pygame.draw.rect(screen, UI.CHARCOAL_200, wipe_btn_rect, width=1, border_radius=2)
            ...
            if is_delete_confirm:
                ...
                delete_bg = UI.ERROR if delete_hover else UI.ERROR_DIM
                pygame.draw.rect(screen, delete_bg, confirm_rect, border_radius=3)
                pygame.draw.rect(screen, UI.CORAL_600, confirm_rect, width=2, border_radius=3)
                
                # Draw "¿delete?" text centered
                delete_text = font.render("¿delete?", True, UI.WHITE)
                ...
                pygame.draw.rect(screen, UI.CHARCOAL_300,
                                 item_rect, border_radius=3)
                pygame.draw.rect(screen, UI.AZURE_600,
                                 item_rect, width=2, border_radius=3)
                ...
                edit_surf_before = font.render(
                    text_before_cursor, True, UI.WHITE)
                edit_surf_after = font.render(
                    text_after_cursor, True, UI.WHITE)
                
                # Calculate positions (text vertically centred in the row so
                # descenders like '_' stay clear of the fill's bottom edge)
                before_x = item_rect.x + 5
                before_y = item_rect.y + (item_rect.height - edit_surf_before.get_height()) // 2
                ...
                if self.dropup_edit_selection_start != -1:
                    ...
                    sel_surf_before = font.render(text_before_sel, True, UI.WHITE)
                    sel_surf = font.render(text_selected, True, UI.WHITE)
                    ...
                    pygame.draw.rect(screen, UI.COBALT_800, sel_rect)
                ...
                if self.dropup_edit_cursor_visible:
                    ...
                    pygame.draw.rect(screen, UI.WHITE, cursor_rect)
            else:
                ...
                if is_editing:
                    ...
                    if is_current and is_keyboard_selected:
                        ...
                        pygame.draw.rect(screen, UI.OCEAN_600,
                                         text_area_rect, border_radius=3)
                    elif is_current:
                        # Green background for current generation
                        pygame.draw.rect(screen, UI.JADE_DIM,
                                         text_area_rect, border_radius=3)
                ...
                if not is_editing:
                    ...
                    if text_key not in self._ui_cache:
                        if is_keyboard_selected:
                            ...
                            text_color = UI.AZURE_300
                        elif is_current:
                            text_color = UI.JADE_400  # Light green for current
                        else:
                            text_color = UI.SMOKE_200  # Normal white
                    ...
                    screen.blit(text_surf, (text_x, text_y), (0, 0, text_width, text_surf.get_height()))
    ...
    def _render_counter_surfaces(self, current_frame, total_frames, fps, screen_width, screen_height):
        ...
        # 1. Top line: resolution + colour space (small, secondary)
        res_w, res_h = self.original_resolution
        if res_w > 0 and res_h > 0:
            resolution_text = f"Res: {res_w}x{res_h} | {self.selected_color_space}"
        else:
            resolution_text = f"Res: Unknown | {self.selected_color_space}"
        ...
        # Frame text split in three so the changing digits live in a
        # fixed-width field and never push the "/total | fps" tail around
        if self.channel_mode != "RGB":
            frame_post_text = f"/{total_display_frames} | {fps:.1f} fps | {self.channel_mode}"
        else:
            frame_post_text = f"/{total_display_frames} | {fps:.1f} fps"

        f15 = ui_font(15)
        resolution_surf = f15.render(resolution_text + "  |  ", True, UI.SMOKE_700)
        frame_pre_surf = f15.render("Frame: ", True, UI.SMOKE_200)
        frame_cur_surf = f15.render(str(display_frame), True, UI.SMOKE_200)
        frame_post_surf = f15.render(frame_post_text, True, UI.SMOKE_200)
        # digit field sized for >=4 digits of the widest digit glyph
        cur_field_w = f15.size("8" * max(4, len(str(display_frame)),
                                         len(str(total_display_frames))))[0]

        # v5.3: IN/OUT readout removed from the bottom row (it collided with the
        # toolbar buttons). The marks are shown only as the timeline highlight.
        markers_surf = None

        return {
            'resolution': (resolution_surf, resolution_surf.get_rect()),
            'frame_pre': frame_pre_surf,
            'frame_cur': frame_cur_surf,
            'frame_post': frame_post_surf,
            'cur_field_w': cur_field_w,
    ...
    def _draw_frame_counter_fast(self, screen, counter_surfaces):
        ...
        frame_pre = counter_surfaces['frame_pre']
        frame_cur = counter_surfaces['frame_cur']
        frame_post = counter_surfaces['frame_post']
        cur_field_w = counter_surfaces['cur_field_w']

        # v5.3 r10: single-line readout styled like the buttons beside it,
        # right-aligned so 4-digit "first frame" values grow leftwards
        pill_y = screen_height - UI.DOCK_INSET - 4 - UI.PILL_H
        btn_y = pill_y + (UI.PILL_H - 30) // 2
        right = screen_width - UI.DOCK_INSET - UI.PILL_PAD
        # Every segment has constant width (digits sit right-aligned inside a
        # fixed field), so the box and all text around the digits stay put
        cur_field = max(cur_field_w, frame_cur.get_width())
        text_w = (resolution_rect.width + frame_pre.get_width() +
                  cur_field + frame_post.get_width())
        box = pygame.Rect(right - (text_w + 24), btn_y, text_w + 24, 30)
        pygame.draw.rect(screen, UI.SHADOW, box.move(1, 1), border_radius=UI.RADIUS_BTN)
        self._draw_rounded_outline(screen, box, UI.CHARCOAL_600, UI.ASH_800)
        ty = box.centery - frame_pre.get_height() // 2
        x = box.x + 12
        screen.blit(resolution_surf, (x, ty)); x += resolution_rect.width
        screen.blit(frame_pre, (x, ty)); x += frame_pre.get_width()
        screen.blit(frame_cur, (x + cur_field - frame_cur.get_width(), ty)); x += cur_field
        screen.blit(frame_post, (x, ty))
        return
        ...
        counter_bg_rect = pygame.Rect(
            ...
            row_center_y - counter_bg_height // 2,
            counter_bg_width,
            counter_bg_height
        )
        ...
        if markers_surf and markers_rect:
            ...
            markers_bg_rect = pygame.Rect(
                ...
                row_center_y - markers_bg_height // 2,
                markers_bg_width,
                markers_bg_height
            )
        else:
        ...
        if markers_bg_rect:
            resolution_bg_rect = pygame.Rect(
                ...
                row_center_y - resolution_bg_height // 2,
                resolution_bg_width,
        ...
        # Readouts sub-pill behind the boxes
        pill_left = resolution_bg_rect.left - UI.PILL_PAD
        pill_rect = pygame.Rect(pill_left, pill_y,
                                (right_edge + UI.PILL_PAD) - pill_left, UI.PILL_H)
        pygame.draw.rect(screen, UI.CHARCOAL_700, pill_rect, border_radius=UI.RADIUS_PILL)

        # Draw all three boxes with backgrounds
        boxes = [
            (resolution_bg_rect, UI.CHARCOAL_600, resolution_surf, resolution_rect),
            (counter_bg_rect, UI.CHARCOAL_600, counter_surf, counter_rect)
        ]

        if markers_surf and markers_rect and markers_bg_rect:
            boxes.insert(1, (markers_bg_rect, UI.ERROR_DIM,
                         markers_surf, markers_rect))

        # Draw each box
        for bg_rect, bg_color, text_surf, text_rect in boxes:
            # Draw background
            pygame.draw.rect(screen, bg_color, bg_rect, border_radius=UI.RADIUS_BTN)
    ...
    def _render_timeline_surface(self, screen, current_frame, total_frames):
        ...
        timeline_margin = UI.DOCK_INSET + UI.PILL_PAD  # align with sub-pills

        # Timeline position (top tier of the dock, below the playhead head)
        timeline_top = vh - self.toolbar_height + 8
        ...
        pygame.draw.rect(timeline_surface, UI.CHARCOAL_400, (0, 0,
                         timeline_rect.width, timeline_rect.height), border_radius=4)
        pygame.draw.rect(timeline_surface, UI.CHARCOAL_200, (0, 0,
        ...
        if self.user_marks_active and total_frames > 0:
            ...
            if marked_width > 0:
                ...
                pygame.draw.rect(timeline_surface, UI.JADE_600,
                ...
                pygame.draw.rect(timeline_surface, UI.PLAYHEAD_RED,
    ...
    def _update_timeline_indicator(self, timeline_surface, current_frame, total_frames):
        ...
        if self.user_marks_active and total_frames > 0:
            ...
            pygame.draw.rect(updated_surf, UI.GOLD_500,
            ...
            pygame.draw.rect(updated_surf, UI.PLAYHEAD_RED,
                             out_marker, border_radius=1)  # Red

        return (updated_surf, rect)

    # -----------------------
    # v5.3 ComfyUI skin: floating dock layout
    # One source of truth for button positions, read by BOTH the draw loop
    # and the hover loop so they can never drift apart.
    # -----------------------
    DOCK_GROUPS = (
        ("in", "back", "fps_control", "forward", "out", "Pong"),
        ("1:1", "Fit", "Width", "Height", "Fullscreen", "Reset"),
        ("Generations", "Snapshot", "ClearCache", "Vision"),
    )
    VIEW_ICON_BUTTONS = ("1:1", "Fit", "Width", "Height", "Fullscreen", "Reset")

    def _draw_icon_button(self, screen, name, position, is_active=False, is_hover=False):
        """30x30 icon button for the view group (Lucide-style thin glyphs).
        Rendered supersampled at 4x for smooth corners and strokes, cached."""
        btn_rect = pygame.Rect(position[0], position[1], 30, 30)
        shadow_rect = btn_rect.move(1, 1)
        pygame.draw.rect(screen, UI.SHADOW, shadow_rect, border_radius=UI.RADIUS_BTN)

        if is_active:
            bg, border = UI.CHARCOAL_300, UI.AZURE_600
        elif is_hover:
            bg, border = UI.CHARCOAL_400, UI.ASH_800
        else:
            bg, border = UI.CHARCOAL_600, UI.ASH_800

        key = ("iconbtn", name, bg, border)
        surf = self._ui_cache.get(key)
        if surf is None:
            S = 4
            big = pygame.Surface((30 * S, 30 * S), pygame.SRCALPHA)
            pygame.draw.rect(big, border, big.get_rect(), border_radius=UI.RADIUS_BTN * S)
            inner = big.get_rect().inflate(-2 * UI.STROKE_BTN * S, -2 * UI.STROKE_BTN * S)
            pygame.draw.rect(big, bg, inner, border_radius=(UI.RADIUS_BTN - UI.STROKE_BTN) * S)
            ink, lw = UI.SMOKE_200, 6  # 1.5px at 1x
            L = pygame.draw.line
            if name == "Fit":          # frame with inward chevrons
                pygame.draw.rect(big, ink, pygame.Rect(30, 34, 60, 52), width=lw, border_radius=6)
                for x0, dx in ((52, -10), (68, 10)):
                    L(big, ink, (x0, 46), (x0 + dx, 60), lw); L(big, ink, (x0 + dx, 60), (x0, 74), lw)
            elif name == "Width":      # horizontal double arrow
                L(big, ink, (24, 60), (96, 60), lw)
                L(big, ink, (38, 46), (24, 60), lw); L(big, ink, (24, 60), (38, 74), lw)
                L(big, ink, (82, 46), (96, 60), lw); L(big, ink, (96, 60), (82, 74), lw)
            elif name == "Height":     # vertical double arrow
                L(big, ink, (60, 24), (60, 96), lw)
                L(big, ink, (46, 38), (60, 24), lw); L(big, ink, (60, 24), (74, 38), lw)
                L(big, ink, (46, 82), (60, 96), lw); L(big, ink, (60, 96), (74, 82), lw)
            elif name == "Fullscreen":  # four corner brackets
                for (cx, cy, dx, dy) in ((32, 32, 1, 1), (88, 32, -1, 1), (32, 88, 1, -1), (88, 88, -1, -1)):
                    L(big, ink, (cx, cy), (cx + 16 * dx, cy), lw)
                    L(big, ink, (cx, cy), (cx, cy + 16 * dy), lw)
            elif name == "Reset":      # rotate-ccw: ring with a 90-degree gap + arrowhead
                glyph = pygame.Surface(big.get_size(), pygame.SRCALPHA)
                pygame.draw.circle(glyph, ink, (62, 64), 24, lw)
                pygame.draw.polygon(glyph, (0, 0, 0, 0),
                                    [(62, 64), (62, 8), (8, 8), (8, 64)])
                pygame.draw.polygon(glyph, ink, [(60, 28), (60, 52), (40, 40)])
                big.blit(glyph, (0, 0))
            surf = pygame.transform.smoothscale(big, (30, 30))
            if name == "1:1":
                label = ui_font(14).render("1:1", True, UI.SMOKE_200)
                surf.blit(label, label.get_rect(center=(15, 15)))
            self._ui_cache[key] = surf
        screen.blit(surf, btn_rect.topleft)
        return btn_rect

    def _close_all_dropups(self):
        """Dismiss every dropup menu and any in-menu editing state."""
        self.dropup_open = False
    ...
    def _dock_layout(self, vw, vh):
        """Return (button_rects: dict name->Rect, pill_rects: list[Rect]).
        Button sizes mirror the draw helpers exactly: circles 30x30,
        arrows/Pong 40x30 (Pong = text+24 wide), text buttons text+24 x 30."""
        key = (vw, vh)
        cached = self._ui_cache.get("dock_layout")
        ...
        pill_y = vh - UI.DOCK_INSET - 4 - UI.PILL_H          # 4px above dock bottom
        btn_y = pill_y + (UI.PILL_H - 30) // 2
        x = UI.DOCK_INSET + UI.PILL_PAD                       # dock left + first pill pad
        rects, pills = {}, []
        for group in self.DOCK_GROUPS:
            pill_left = x
            x += UI.PILL_PAD
            for b in group:
                if b in ("in", "out", "fps_control"):
                    ...
                    w, step = 40, 50
                elif b in self.VIEW_ICON_BUTTONS:
                    w, step = 30, 40
                else:
                    w = font.size(b)[0] + 24
                    step = w + 10
                rects[b] = pygame.Rect(x, btn_y, w, 30)
                x += step
            pill_right = x - 10 + UI.PILL_PAD                 # drop trailing spacing
            pills.append(pygame.Rect(pill_left, pill_y, pill_right - pill_left, UI.PILL_H))
            x = pill_right + UI.PILL_GAP
        self._ui_cache["dock_layout"] = (key, rects, pills)
        return rects, pills

    def _draw_rounded_outline(self, surface, rect, fill, stroke=None, radius=None, width=None):
        """Antialiased rounded rect (optionally with an outline ring).
        pygame draws unantialiased corners, so render at 4x and smoothscale
        down; results are cached per (size, colours, radius, stroke)."""
        r = UI.RADIUS_BTN if radius is None else radius
        w = UI.STROKE_BTN if width is None else width
        key = ("aarr", rect.width, rect.height, tuple(fill), tuple(stroke) if stroke else None, r, w)
        surf = self._ui_cache.get(key)
        if surf is None:
            S = 4
            big = pygame.Surface((rect.width * S, rect.height * S), pygame.SRCALPHA)
            if stroke is not None:
                pygame.draw.rect(big, stroke, big.get_rect(), border_radius=r * S)
                inner = big.get_rect().inflate(-2 * w * S, -2 * w * S)
                pygame.draw.rect(big, fill, inner, border_radius=max(1, (r - w) * S))
            else:
                pygame.draw.rect(big, fill, big.get_rect(), border_radius=r * S)
            surf = pygame.transform.smoothscale(big, (rect.width, rect.height))
            self._ui_cache[key] = surf
        surface.blit(surf, rect.topleft)

    def _render_toolbar_surface(self, screen):
        """Pre-render the dock shell (floating rounded panel, ComfyUI actionbar style)"""
        vw, vh = screen.get_size()
        th = self.toolbar_height
        inset = UI.DOCK_INSET

        toolbar_surface = pygame.Surface((vw, th), pygame.SRCALPHA)
        dock = pygame.Rect(inset, 0, vw - 2 * inset, th - inset)

        # soft drop shadow (a few expanding translucent rects)
        for i, a in ((3, 18), (2, 30), (1, 45)):
            pygame.draw.rect(toolbar_surface, (0, 0, 0, a),
                             dock.inflate(i * 2, i * 2).move(0, i),
                             border_radius=UI.RADIUS_DOCK + i)

        if self.vision_module_open:
            # Vision tier sits on top and draws the rounded cap; keep our top square
            pygame.draw.rect(toolbar_surface, UI.TOOLBAR_ALPHA, dock,
                             border_bottom_left_radius=UI.RADIUS_DOCK,
                             border_bottom_right_radius=UI.RADIUS_DOCK)
            pygame.draw.rect(toolbar_surface, UI.DOCK_STROKE_ALPHA, dock, width=1,
                             border_bottom_left_radius=UI.RADIUS_DOCK,
                             border_bottom_right_radius=UI.RADIUS_DOCK)
            # erase the top stroke so the two tiers read as one shell
            pygame.draw.line(toolbar_surface, UI.TOOLBAR_ALPHA,
                             (dock.left + 1, 0), (dock.right - 2, 0), 1)
        else:
            pygame.draw.rect(toolbar_surface, UI.TOOLBAR_ALPHA, dock, border_radius=UI.RADIUS_DOCK)
            pygame.draw.rect(toolbar_surface, UI.DOCK_STROKE_ALPHA, dock, width=1, border_radius=UI.RADIUS_DOCK)
        self._toolbar_surface_vision_state = self.vision_module_open
    ...
    def _draw_timeline_scrubber(self, screen, current_frame, total_frames, mouse_pos):
        ...
        if self.viewer_paused and total_frames > 0 and self.timeline_extended_rect:
            ...
            # v5.3 ComfyUI skin: NLE-style playhead - 2px line + tab head
            # (flat top, pointed base), no bottom triangle.
            pygame.draw.line(screen, UI.AZURE_600,
                           (indicator_x, timeline_rect.top),
                           (indicator_x, timeline_rect.bottom - 2),
                           width=2)

            head_top = timeline_rect.top - 6
            head_points = [
                (indicator_x - 5, head_top),          # top-left
                (indicator_x + 5, head_top),          # top-right
                (indicator_x + 5, head_top + 5),      # right shoulder
                (indicator_x, head_top + 10),         # point
                (indicator_x - 5, head_top + 5),      # left shoulder
            ]
            pygame.draw.polygon(screen, UI.AZURE_600, head_points)
            ...
            while self.running:
                ...
                if not display_initialized or self.new_content_ready.is_set():
                    ...
                    if screen is None or not display_initialized:
                        ...
                        pygame.display.set_caption(
                            "Preview Video Monitor Pro v5.3 - [q] Close | [SPACE] Play/Pause | [←→] Step | [ENTER] Edit/Confirm | [↑↓] Navigate | [END] Load | [1-5] Fit Modes | [i/o/p] In/Out Marks | [W] Wipe Toggle | Click timeline to scrub | Right-click generation to rename")
                ...
                while self.running and not self.new_content_ready.is_set():
                    ...
                    for event in events:
                            ...
                            if self.fps_custom_editing:
                                if event.key == pygame.K_BACKSPACE:
                                    ...
                                    if event.unicode == '.' and '.' in self.fps_custom_text:
                                        ...
                                        self._mark_ui_dirty()
                                # Field is modal: no key typed here may fall through to
                                # global hotkeys (digits are fit modes, r/g/b channels...)
                                continue
                        ...
                        if event.type == pygame.MOUSEWHEEL:
                            ...
                            if (self.dropup_open or self.color_space_dropup_open or 
                                self.snapshot_dropup_open or self.clearcache_dropup_open or
                                self.fps_dropup_open):
                                ...
                                if self.sbs_active:
                                    ...
                                    if event.y != 0:
                                        ...
                                        vw, vh = screen.get_size()
                                        half_width = vw // 2
                                        th = 0  # glass dock: video uses the full window
                                        available_height = vh - th
                        ...
                        if event.type == pygame.MOUSEBUTTONDOWN:
                            ...
                            # Click on a menu's own toggle button? Then the button handler
                            # (render section) owns the toggle: it must see the menu still
                            # open so a second click CLOSES it instead of dismiss+reopen.
                            _menu_owner_buttons = ("Generations", "Snapshot", "ClearCache", "fps_control")
                            click_on_menu_button = any(
                                b in _menu_owner_buttons and rect.collidepoint(event.pos)
                            ...
                            if event.button == 1:  # Left click
                                ...
                                # Check if clicking on extended timeline area for scrubbing
                                    ...
                                    if not click_on_menu_button and (self.dropup_open or self.color_space_dropup_open or
                                            self.snapshot_dropup_open or self.clearcache_dropup_open or
                                            self.fps_dropup_open):
                                        # First click dismisses open menus; it never scrubs through them
                                        event_handled_by_dropup = True
                                        self._close_all_dropups()
                                        continue
                                    self.scrubbing = True
                                    rel_x = event.pos[0] - \
                                ...
                                # FIXED: Wipe line click handling with proper video-relative position calculation
                                    ...
                                    if not click_on_menu_button and (self.dropup_open or self.color_space_dropup_open or
                                            self.snapshot_dropup_open or self.clearcache_dropup_open or
                                            self.fps_dropup_open):
                                        # Click outside an open menu: dismiss it, never play/act through it
                                        event_handled_by_dropup = True
                                        self._close_all_dropups()
                                        continue
                                ...
                                if (self.vision_module_open and event.button == 1 and 
                                    ...
                                    not self.clearcache_dropup_open and
                                    not self.fps_dropup_open):
                                    vw, vh = screen.get_size()
                                    module_height = 50
                                    ...
                                    if event.pos[1] >= module_y and event.pos[1] <= module_y + module_height:
                                        ...
                                        padding = 10
                                        bar_y = module_y + 15 + 20  # matches _draw_vision_slider
                                        bar_x_start = 10 + padding + 5
                                        bar_x_end = 10 + slider_width - padding - 5
                                        bar_width = bar_x_end - bar_x_start
                                        ...
                                        gamma_bar_x_start = 10 + slider_width + padding + 5 + 5
                                        gamma_bar_x_end = 10 + slider_width * 2 - padding - 5 - 5
                                        ...
                                        sat_bar_x_start = 10 + slider_width * 2 + padding + 5 + 5
                                        sat_bar_x_end = 10 + slider_width * 3 - padding - 5 - 5
                            ...
                            elif event.button == 2 and (self.color_space_dropup_open or
                                    self.snapshot_dropup_open or self.clearcache_dropup_open or
                                    self.fps_dropup_open or
                                    (self.dropup_open and not (dropup_rect and dropup_rect.collidepoint(event.pos)))):
                                # A menu is open: dismiss it; never reset view/sliders through it
                                self._close_all_dropups()
                                ...
                                if self.dropup_open and dropup_rect and dropup_rect.collidepoint(event.pos):
                                    ...
                                    if self.vision_module_open:
                                        ...
                                        if event.pos[1] >= module_y and event.pos[1] <= module_y + module_height:
                                            ...
                                            slider_width = slider_section_width // 3
                                            padding = 10
                                            bar_y = module_y + 15 + 20
                                            
                                            # Check GAIN slider
                                            gain_bar_x_start = 10 + padding + 5 + 5
                                            gain_bar_x_end = 10 + slider_width - padding - 5 - 5
                        ...
                        if event.type == pygame.MOUSEMOTION:
                            ...
                            # PRIORITY 1: Vision module slider dragging (only if NO dropups open!)
                                ...
                                if self.vision_dragging_slider == "gain":
                                    ...
                                    bar_x_start = 10 + slider_width + padding + 5
                                    bar_x_end = 10 + slider_width * 2 - padding - 5
                                    bar_width = bar_x_end - bar_x_start
                                    ...
                                    bar_x_start = 10 + slider_width * 2 + padding + 5
                                    bar_x_end = 10 + slider_width * 3 - padding - 5
                                    bar_width = bar_x_end - bar_x_start
                    ...
                    if mouse_moved and not self.fullscreen_mode and screen is not None:
                        ...
                        # v5.3 dock: positions from the shared layout (single row)
                        _hov_rects, _ = self._dock_layout(screen.get_width(), screen.get_height())
                        btn_y_circular = _hov_rects["in"].y
                        btn_y_text = _hov_rects["in"].y
                        ...
                        for b in btns:
                            ...
                            btn_x = _hov_rects[b].x  # dock layout (groups / pills)

                            btn_rect = _hov_rects[b]
                            if b in ["in", "out", "fps_control"]:
                    ...
                    if screen is not None:
                        ...
                        if not self.fullscreen_mode and screen is not None:
                            ...
                            if (self._cached_toolbar_surface is None or
                                    getattr(self, "_toolbar_surface_vision_state", None) != self.vision_module_open):
                                self._cached_toolbar_surface = self._render_toolbar_surface(screen)
                            
                            # Blit pre-rendered dock shell
                            if self._cached_toolbar_surface:
                                screen.blit(self._cached_toolbar_surface,
                                            (0, vh - self.toolbar_height))

                            # Grouped layout (sub-pill backgrounds removed in r10:
                            # buttons read fine directly on the glass dock)
                            dock_rects, _dock_pills = self._dock_layout(vw, vh)
                            ...
                            # v5.3 dock: positions come from _dock_layout (single row)
                            btn_x = 10
                            btn_y_circular = dock_rects["in"].y
                            btn_y_text = dock_rects["in"].y
                            ...
                            for b in btns_draw_order:
                                ...
                                if b in text_buttons:
                                    ...
                                    # Use original position for circular/arrow/Pong buttons (Group 1)
                                    current_y = btn_y_circular
                                btn_x = dock_rects[b].x  # dock layout (groups / pills)
                                ...
                                if b == "in":
                                    ...
                                    btn_rect = self._draw_button(
                                        screen, b, (btn_x, current_y), is_active, is_hover, UI.AZURE_600)
                                    btn_rects.append((b, btn_rect))
                                    ...
                                    color_override = None
                                    if b == "ClearCache":
                                        color_override = UI.AZURE_600
                                    elif b == "Generations":
                                        color_override = UI.AZURE_600
                                    elif b == "Snapshot":
                                        # Dark green when dropup open, cyan when snapshot taken, grey otherwise
                                        if self.snapshot_dropup_open:
                                            color_override = UI.AZURE_600  # dropup open
                                        elif is_snapshot_active:
                                            color_override = UI.AZURE_400  # brief flash when snapshot taken (lighter azure)
                                        # else: None = grey outline (default)
                                    elif b == "Vision":
                                        color_override = UI.AZURE_600  # Blue outline

                                    if b in self.VIEW_ICON_BUTTONS:
                                        btn_rect = self._draw_icon_button(
                                            screen, b, (btn_x, current_y), is_active, is_hover)
                                    else:
                                        btn_rect = self._draw_button(
